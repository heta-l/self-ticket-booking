...
const Events = () => {
  ...
  const loadEvents = async () => {
    // SIMULATED: const res = await initials ApiService.post("/events/list", {})
                ...
                <div>
                  ...
                  <div className="mt-6">
                    ...
                    <div className="flex justify-between text-xs text-gray-500 mb-1">
                      ...
                      <span>{Math.max(0, 100 - Math.round((parseInt(/*event.bookingCount*/0)  / event.capacity) * 100))}%</span>
                    </div>
                    <div className="w-full h-2 rounded-full bg-gray-200">
                      <div
                        className="h-2 bg-orange-500 rounded-full"
                        style={{ width: `${Math.min(100, Math.max(0, 100 - (parseInt(/*event.bookingCount*/ 0) / event.capacity) * 100))}%` }}
                      ></div>
                    </div>
                  </div>
                </div>
